...
# 配置
# ============================================================
DATA_PATH = '/ssd/data/agent/bio/eacn_example_001/experiments/np_guard/data/subset.h5ad'
...
import harmonypy
ho = harmonypy.run_harmony(adata.obsm['X_pca'], adata.obs, 'Dataset')
import numpy as np_tmp
z_harmony = np_tmp.array(ho.Z_corr)
if z_harmony.shape[0] != adata.n_obs: z_harmony = z_harmony.T
...
# Fix NaN/Inf and normalize features
features = np.nan_to_num(features, nan=0.0, posinf=10.0, neginf=-10.0)
from sklearn.preprocessing import StandardScaler
features = StandardScaler().fit_transform(features)
print(f"Features: shape={features.shape}, nan={np.isnan(features).sum()}, inf={np.isinf(features).sum()}")
...
for name, z in [('harmony', z_harmony), ('linear', z_linear),
    ...
    try:
        asw = silhouette_score(z[:5000], adata.obs['Dataset'].values[:5000],
                               sample_size=5000)
